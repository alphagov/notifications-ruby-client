...
def main
  client = Notifications::Client.new(ENV['API_CLIENT_INTEGRATION_TESTS_TEST_API_KEY'], ENV['API_CLIENT_INTEGRATION_TESTS_NOTIFY_API_URL'])
  test_get_email_template_by_id(client, ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL_TEMPLATE_ID'])
  test_get_sms_template_by_id(client, ENV['API_CLIENT_INTEGRATION_TESTS_SMS_TEMPLATE_ID'])
  test_get_letter_template_by_id(client, ENV['API_CLIENT_INTEGRATION_TESTS_LETTER_TEMPLATE_ID'])
  test_get_template_version(client, ENV['API_CLIENT_INTEGRATION_TESTS_SMS_TEMPLATE_ID'], 1)
  test_get_all_templates(client)
  test_get_all_templates_filter_by_type(client)
  test_generate_template_preview(client, ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL_TEMPLATE_ID'])
...
def test_send_email_endpoint(client)
  email_resp = client.send_email(
    email_address: ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL'],
    template_id: ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL_TEMPLATE_ID'],
    personalisation: { "name" => "some name" },
    reference: "some reference",
    email_reply_to_id: ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL_REPLY_TO_ID'],
...
def test_send_email_endpoint_with_document(client)
  email_resp = File.open('spec/test_files/test_pdf.pdf', 'rb') do |f|
    client.send_email(email_address: ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL'],
      template_id: ENV['API_CLIENT_INTEGRATION_TESTS_EMAIL_TEMPLATE_ID'],
...
def test_send_sms_endpoint(client)
  sms_resp = client.send_sms(phone_number: ENV['API_CLIENT_INTEGRATION_TESTS_NUMBER'], template_id: ENV['API_CLIENT_INTEGRATION_TESTS_SMS_TEMPLATE_ID'],
                             personalisation: { "name" => "some name" },
                             reference: "some reference",
                             sms_sender_id: ENV['API_CLIENT_INTEGRATION_TESTS_SMS_SENDER_ID'])
...
def test_send_letter_endpoint(client)
  letter_resp = client.send_letter(
    template_id: ENV['API_CLIENT_INTEGRATION_TESTS_LETTER_TEMPLATE_ID'],
    personalisation: {
...
def test_get_received_texts
  client = Notifications::Client.new(ENV['API_CLIENT_INTEGRATION_TESTS_INBOUND_SMS_API_KEY'], ENV['API_CLIENT_INTEGRATION_TESTS_NOTIFY_API_URL'])
